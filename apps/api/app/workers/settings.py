...
from app.services.proxy import expire_due_proxy_access, sync_socks_users
...
async def expire_proxy(ctx: dict) -> int:
    async with SessionLocal() as db:
        expired = await expire_due_proxy_access(db)
        await sync_socks_users(db, ctx["settings"])
        await db.commit()
        return expired
...
class WorkerSettings:
    functions = [expire_subscriptions, expire_proxy, health_check_nodes]
    cron_jobs = [
        cron(expire_subscriptions, minute={0, 15, 30, 45}),
        cron(expire_proxy, minute={0, 15, 30, 45}),
